...
distributed. This could result in a poor fit. 
```{r, eval = FALSE}
...
Maybe a cubic polynomial would be better? Looking at the model, we immediately see 
an improvement. 
```{r, eval = FALSE}
...
Next, we try stepwise regression. Again, we see improvement from the linear model. 
```{r, eval = FALSE}
...
cap_Python8 <- "Figure 1.8 Age v. Wage (Stepwise)"
``` 
...
cap_Python9 <- "Figure 1.9 Stepwise Model Summary"
``` 

Finally, we move on to splines. 
```{r, eval = FALSE}
